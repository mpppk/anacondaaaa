...
func TestAccountActivityEvents(t *testing.T) {
	tests := []struct {
		name     string
		fileName string
		want     anacondaaaa.AccountActivityEvent
	}{
		{
		...
		},
		{
			...
			want: anacondaaaa.AccountActivityEvent{
				ForUserId: "2244994945",
				FavoriteEvents: []*anacondaaaa.FavoriteEvent{
					{
		...
		},
		{
			...
			want: anacondaaaa.AccountActivityEvent{
				ForUserId: "2244994945",
				FollowEvents: []*anacondaaaa.UserRelationEvent{
					{
						Type:             anacondaaaa.EventType(anacondaaaa.FollowEventType),
		...
		},
		{
			...
			want: anacondaaaa.AccountActivityEvent{
				...
				FollowEvents: []*anacondaaaa.UserRelationEvent{
					{
						Type:             anacondaaaa.EventType(anacondaaaa.UnfollowEventType),
						CreatedTimestamp: "1517588749178",
						Target:           &anaconda.User{Id: 1},
						Source:           &anaconda.User{Id: 2},
					},
		...
		},
		{
			...
			want: anacondaaaa.AccountActivityEvent{
				ForUserId: "2244994945",
				BlockEvents: []*anacondaaaa.UserRelationEvent{
					{
						Type:             anacondaaaa.EventType(anacondaaaa.BlockEventType),
		...
		},
		{
			...
			want: anacondaaaa.AccountActivityEvent{
				...
				BlockEvents: []*anacondaaaa.UserRelationEvent{
					{
						Type:             anacondaaaa.EventType(anacondaaaa.UnblockEventType),
						CreatedTimestamp: "1518127020304",
						Target:           &anaconda.User{Id: 1},
						Source:           &anaconda.User{Id: 2},
					},
		...
		},
		{
			...
			want: anacondaaaa.AccountActivityEvent{
				ForUserId: "2244994945",
				MuteEvents: []*anacondaaaa.UserRelationEvent{
					{
						Type:             anacondaaaa.EventType(anacondaaaa.MuteEventType),
						CreatedTimestamp: "1518127020304",
						Target:           &anaconda.User{Id: 1},
						Source:           &anaconda.User{Id: 2},
					},
		...
		},
		{
			...
			want: anacondaaaa.AccountActivityEvent{
				...
				MuteEvents: []*anacondaaaa.UserRelationEvent{
					{
						Type:             anacondaaaa.EventType(anacondaaaa.UnmuteEventType),
						CreatedTimestamp: "1518127020304",
						Target:           &anaconda.User{Id: 1},
						Source:           &anaconda.User{Id: 2},
					},
		...
		},
		{
			...
			want: anacondaaaa.AccountActivityEvent{
				UserEvent: &anacondaaaa.UserEvent{
					Revoke: &anacondaaaa.Revoke{
		...
		},
		{
			...
			want: anacondaaaa.AccountActivityEvent{
				...
				DirectMessageEvents: []*anacondaaaa.DirectMessageEvent{
					{
						Type:             anacondaaaa.EventType(anacondaaaa.MessageCreateEventType),
						Id:               "954491830116155396",
						CreatedTimestamp: "1516403560557",
						MessageCreate: &anacondaaaa.MessageCreate{
							Target: struct {
								RecipientId string `json:"recipient_id"`
							}{"4337869213"},
							SenderId:    "3001969357",
							SourceAppId: "13090192",
							MessageData:anacondaaaa.MessageData{
								Text: "Hello World!",
								Entities: &anacondaaaa.Entities{
									Hashtags:     []interface{}{},
									Symbols:      []interface{}{},
		...
		},
		{
			...
			want: anacondaaaa.AccountActivityEvent{
				ForUserId: "4337869213",
				DirectMessageIndicateTypingEvents: []*anacondaaaa.DirectMessageIndicateTypingEvent{
					{
		...
		},
		{
			...
			want: anacondaaaa.AccountActivityEvent{
				ForUserId: "4337869213",
				DirectMessageMarkReadEvents: []*anacondaaaa.DirectMessageMarkReadEvent{
					{
		...
		},
		{
			...
			want: anacondaaaa.AccountActivityEvent{
				...
				TweetDeleteEvents: []*anacondaaaa.TweetDeleteEvent{
					{
						Status: struct {
							Id     string `json:"id"`
							UserId string `json:"user_id"`
...
func generateTweetEventTestFunc(filePath string, want *anacondaaaa.AccountActivityEvent) func(t *testing.T) {
	return func(t *testing.T) {
		...
		var aae anacondaaaa.AccountActivityEvent
		...
		if diff := cmp.Diff(aae, *want); diff != "" {
			t.Errorf("anacondaaaa.AccountActivityEvent differs: (-got +want)\n%s", diff)
		}
	}
}
...
func TestAccountActivityEvent_GetApps(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			fileName: "direct_message_events.json",
			want: anacondaaaa.Apps{
				SourceApp: &anacondaaaa.SourceApp{
					Id:   "13090192",
					Name: "FuriousCamperTestApp1",
					URL:  "https://twitter.com/furiouscamper",
				},
				Sender: &anacondaaaa.AppUser{
					Id:                   "3001969357",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("anacondaaaa.AccountActivityEvent.GetApps() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if diff := cmp.Diff(got, &tt.want); diff != "" {
				t.Errorf("anacondaaaa.AccountActivityEvent.Apps differs: (-got +want)\n%s", diff)
			}
		})
	}
}
...
func TestAccountActivityEvent_GetUsers(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("anacondaaaa.AccountActivityEvent.GetUsers() error = %v, wantErr %v", err, tt.wantErr)
				return
			}

			compare := func(gotUser *anacondaaaa.AppUser, wantUsers []*anacondaaaa.AppUser) {
				for _, wantUser := range wantUsers {
					if gotUser.Id == wantUser.Id {
						if diff := cmp.Diff(gotUser, wantUser); diff != "" {
							t.Errorf("anacondaaaa.AccountActivityEvent.Users differs: (-got +want)\n%s", diff)
						}
						return
					}
				}
